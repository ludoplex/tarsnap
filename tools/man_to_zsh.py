...
_AUTO_GENERATED_US = (
    f"{_AUTO_GENERATED}, then modified by {os.path.basename(__file__)}"
)
...
def write_zsh(filename_zsh, options, optlist, descs):
    ...
    if shtab.__version__ != _HANDLES_SHTAB_VERSION:
        print(
            f"Script designed for shtab {_HANDLES_SHTAB_VERSION}; found {shtab.__version__} instead"
        )
        exit(1)
    ...
    zsh_output = f"{zsh_output[:index]}{_ZSH_PREAMBLE}{zsh_output[index:]}"
